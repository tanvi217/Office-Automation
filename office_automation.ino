...
void loop() {
  ...
  if(command=="auto_on" && analogRead(ir_sensor)==LOW)
  {
  ...
  if(command=="manual")
  {
        ...
        manual_value=Serial.read();
        analogWrite(bulb_read, manual_value); 
  } 
  if(mv<tempMin && digitalRead(ir_sensor)==LOW)
  {
  Serial.print("Fan Speed");
  Serial.print(fanSpeed);
  analogWrite(fan,fanSpeed);
  }
  else if((mv >= tempMin) && (mv <= tempMax) && digitalRead(ir_sensor) == LOW) //if temperature is higher than the minimum range
  {
  ...
  else if(mv>tempMax && digitalRead(ir_sensor) == LOW)
  {
  Serial.print("Fan speed ");
  Serial.println("255");
  analogWrite(fan, 255);
